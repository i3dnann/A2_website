...
- Never commit `.env`, API keys, tokens, Discord secrets, Steam secrets, webhook URLs, database passwords, private keys, logs, `node_modules`, `dist`, or `build`.
...
- Discord webhooks run only on the backend.
- CFW character lookup is read-only for normal users and is limited to identifiers linked to the logged-in account.
