          ...
          {content.news.map((n) => {
            ...
            return (
              <motion.a href="#" key={n.title} variants={staggerItem} whileHover={{ y: -6 }} className="group flex flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition-colors hover:border-orange-400/30">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-orange-300">Read more <ArrowUpRight size={14} className="transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></span>
              </motion.a>
            );
          })}
        </motion.div>
