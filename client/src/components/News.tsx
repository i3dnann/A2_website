    ...
    <section id="news" className="relative py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Reveal className="mx-auto max-w-2xl text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-orange-400">{t(content.newsSubtitle)}</p>
          ...
          {content.news.map((n) => {
            ...
            return (
              <motion.a href="/news" key={n.id || n.title} variants={staggerItem} whileHover={{ y: -6 }} className="spotlight-card group flex flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition-colors hover:border-orange-400/30">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-orange-300">{t("Read more")} <ArrowUpRight size={14} className={`transition-transform group-hover:-translate-y-1 ${isArabic ? "rotate-180 group-hover:-translate-x-1" : "group-hover:translate-x-1"}`} /></span>
              </motion.a>
            );
          })}
        </motion.div>}
