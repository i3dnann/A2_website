          ...
          <div>
            ...
            <div className="mt-5 flex gap-3">
              {SOCIAL_LINKS.map(({ icon: Icon, label, path }) => {
                ...
                return (
                  <a key={label} aria-label={label} href={href} target={external ? "_blank" : undefined} rel={external ? "noreferrer" : undefined} className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 text-white/60 transition hover:border-orange-400/40 hover:text-orange-300"><Icon size={16} /></a>
                );
              })}
            </div>
          </div>
