...
export default function ThemeSettingsPage() {
  ...
  const save = async (event) => {
    ...
    setStatus("Theme, fonts, and animations saved.");
  };
  ...
  return (
    <div className="grid gap-5">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-black md:text-4xl">Fonts, colors and animations</h1>
        <p className="mt-2 max-w-3xl text-sm text-white/55">Choose website-wide header, text, and button fonts. Google fonts load automatically. Adobe fonts work by adding your Adobe Fonts kit CSS URL.</p>
      </header>
          ...
          <form className="grid gap-6" onSubmit={save}>
            <section className="grid gap-4 rounded-xl border border-a2-border bg-white/[0.03] p-4">
              ...
              <label className="grid gap-2 text-sm font-bold">
                ...
                <span className="text-xs text-white/45">Adobe Fonts cannot be loaded as a full public list. Paste your Adobe kit URL here, then use its font family name in your kit.</span>
              </label>
              <p className="rounded-lg border border-a2-border bg-black/30 p-3 text-xs leading-6 text-white/50">Gotham Regular is included as a font option. To use the real Gotham file, place your licensed font at <code>/fonts/gotham-regular.woff2</code> or <code>/fonts/gotham-regular.otf</code>. Until then it falls back to Montserrat.</p>
            </section>

            <section className="grid gap-4 rounded-xl border border-a2-border bg-white/[0.03] p-4">
              <div className="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-a2-green"><Sparkles size={16} /> Animation type</div>
              <label className="grid gap-2 text-sm font-bold">
                Website animation
                <select className="form-input" value={draft.websiteAnimationType || defaults.websiteAnimationType} onChange={(event) => setField("websiteAnimationType", event.target.value)}>
                  {animationOptions.map((animation) => <option key={animation.value} value={animation.value}>{animation.label}</option>)}
                </select>
              </label>
