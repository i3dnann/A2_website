  ...
  return (
    <PageShell subtitle={content.careersSubtitle} title={content.careersTitle}>
      <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        >
          <p className="max-w-sm text-white/55">{t(content.careersDesc)}</p>
          <a
            href="#positions"
          ...
          >
            {t("View Careers Portal")} <ArrowRight size={16} className={isArabic ? "rotate-180" : ""} />
          </a>
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-serif text-base text-white">{t(career.title || career.role || "Open Position")}</p>
                    <p className="text-xs uppercase tracking-wider text-white/40">{t(career.department || career.dept || "Department")}</p>
                    {career.description && <p className="mt-1 line-clamp-2 max-w-lg text-xs text-white/45">{t(career.description)}</p>}
                  </div>
                </div>
                {isOpen(career) ? (
                  <Link
                  ...
                  >
                    {t("Apply")} <ArrowRight size={13} className={isArabic ? "rotate-180" : ""} />
                  </Link>
                ) : (
