...
function mediaKind(file = {}) {
  ...
  const name = String(
    file.original_name || file.name || file.url || file.stored_name || file.blob_key || ""
  ).toLowerCase();
  ...
  if (
    ...
    /\.(mp3|wav|ogg|oga|m4a|aac|flac)(\?.*)?$/.test(name)
  ) {
    return "audio";
  }
...
function CopyButton({ text }) {
  ...
  return (
    <button
      onClick={copy}
      className="rounded p-1 text-white/45 transition hover:text-white disabled:cursor-not-allowed disabled:opacity-30"
      title="Copy URL"
      type="button"
      disabled={!text}
    >
...
function MediaPreviewModal({ file, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] grid place-items-center bg-black/88 p-4"
      onClick={onClose}
    >
      <div
        className="relative max-h-[92vh] w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-[#08080d] shadow-2xl"
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-white/10 px-3 py-1.5 text-sm font-bold text-white/70 hover:text-white"
          >
            Close
          </button>
        </div>
        ...
        <div className="grid max-h-[78vh] place-items-center overflow-auto bg-black p-4">
          {kind === "image" ? (
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-lg bg-a2-green px-4 py-2 font-black text-black"
            >
              Open file
            </a>
          )}
...
function FileCard({ file, onDelete, onPreview }) {
  ...
  const name = displayName(file);
  const url = fileUrl(file);
  const absoluteUrl = url ? (isAbsoluteUrl(url) ? url : window.location.origin + url) : "";
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#111116] shadow-lg transition hover:border-white/25">
      <button type="button" onClick={() => onPreview(file)} className="block w-full text-left">
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-black/60">
          {kind === "image" && !failed ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center text-white/35">
              <File size={42} />
              <span className="line-clamp-2 text-xs font-bold">Preview unavailable</span>
            </div>
          )}

          <div className="absolute right-2 top-2 rounded-full bg-black/62 p-2 text-white opacity-0 transition group-hover:opacity-100">
            <Eye size={15} />
          </div>
        </div>
      </button>
      ...
      <div className="p-3">
        <p className="line-clamp-2 min-h-[2.25rem] text-sm font-black leading-tight text-white" title={name}>
          {name}
        </p>
...
function UploadZone({ onUploaded }) {
  ...
  const uploadFiles = useCallback(
    async (fileList) => {
      ...
      setUploading(true);
      setProgress(files.map((file) => ({ name: file.name, status: "pending" })));
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setProgress((prev) =>
              prev.map((item, index) => (index === i ? { ...item, status: "error" } : item))
            );
          }
        } catch (error) {
    ...
    <div>
      <div
        ...
        className={`cursor-pointer rounded-2xl border-2 border-dashed p-10 text-center transition ${
          dragging ? "border-a2-green bg-a2-green/10" : "border-white/15 hover:border-white/30 hover:bg-white/3"
        }`}
      >
        <input
          ref={inputRef}
        ...
        {uploading ? (
          <div className="space-y-1.5">
            {progress.map((item, index) => (
              <div key={index} className="flex items-center justify-between gap-3 rounded-lg bg-white/5 px-3 py-2 text-sm">
                <span className="truncate text-white/70">{item.name}</span>

                {item.status === "uploading" && <Loader2 size={14} className="shrink-0 animate-spin text-a2-green" />}
                {item.status === "done" && <Check size={14} className="shrink-0 text-green-400" />}
                {item.status === "error" && <X size={14} className="shrink-0 text-red-400" />}
              </div>
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center gap-3 text-white/40">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm">Images, long audio, videos, PDFs, TXT, and JSON files</p>
            </div>
          </div>
        )}
      </div>
...
export default function MediaLibraryPage() {
  ...
  const filtered = files.filter((file) => {
    ...
    if (filter === "images") return mediaKind(file) === "image";
    if (filter === "audio") return mediaKind(file) === "audio";
    if (filter === "video") return mediaKind(file) === "video";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-white/45">
          {files.length} file{files.length !== 1 ? "s" : ""} stored · {imageCount} image
          {imageCount !== 1 ? "s" : ""} · {audioCount} audio · {videoCount} video
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {["all", "images", "audio", "video"].map((item) => (
          <button
            key={item}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-white/10 py-16 text-white/30">
          ...
          <p className="mt-1 text-sm">Upload files using the area above</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          {filtered.map((file) => (
            <FileCard
              key={file.id || file.url || file.stored_name || file.blob_key}
              file={file}
